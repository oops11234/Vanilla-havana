...
function nightMode(){
  ...
  const pinkNeon = document.querySelectorAll(".neon-effectTxt_pink")
    ...
    const catalog = document.querySelectorAll('.catalog_section a')
    const menu = document.querySelectorAll('.menuItem_section .menuList')
   
    for (let i = 0; i < catalog.length; i++) {
      catalog[i].addEventListener('click', function(){
        menu.forEach(element => {
          element.classList.add("menu_hide")
        });
        menu[i].classList.remove("menu_hide")
      })
